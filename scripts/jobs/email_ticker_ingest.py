...
def _is_valid_market_data(data):
    return data and "error" not in data and "message" not in data


def ingest_premarket(hours=12):
    """Fetch and persist premarket data payload."""
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    pre_market_file = OUT_DIR / "pre_market.json"
    print("Fetching Pre-Market (OpenOutCrier)...")
    pre_market_data = fetch_latest_email("Open Outcrier", sender_email=None, hours=hours)
    if _is_valid_market_data(pre_market_data):
...
def ingest_afterhours(hours=12):
    """Fetch and persist afterhours data payload."""
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    post_market_file = OUT_DIR / "post_market.json"
    print("Fetching Post-Market (FlowAlgo)...")
    post_market_data = fetch_latest_email("FlowAlgo", sender_email="allenw@zgzg.io", hours=hours)
    if _is_valid_market_data(post_market_data):
...
def ingest_emails(job_name="both", hours=12):
    """
    Thin callable wrapper for scheduler/jobs.
    job_name: "premarket" | "afterhours" | "both"
    """
    if job_name == "premarket":
        return {"pre_market": ingest_premarket(hours=hours), "post_market": None}
    if job_name == "afterhours":
        return {"pre_market": None, "post_market": ingest_afterhours(hours=hours)}
    if job_name == "both":
        return {
            "pre_market": ingest_premarket(hours=hours),
            "post_market": ingest_afterhours(hours=hours),
        }
