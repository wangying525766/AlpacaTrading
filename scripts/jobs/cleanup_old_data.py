...
def cleanup_old_files():
    ...
    # Keep existing DATA_DIRS behavior unchanged; attachments use file modified time.
    attachments_path = Path(ATTACHMENTS_DIR)
    if not attachments_path.exists():
        print(f"Directory not found, skipping: {ATTACHMENTS_DIR}")
        return

    for f in attachments_path.iterdir():
        if not f.is_file():
            continue
        try:
            modified_time = dt.datetime.fromtimestamp(f.stat().st_mtime)
            if modified_time < cutoff_date:
                print(f"Deleting old attachment: {f}")
                f.unlink()
        except Exception as e:
            print(f"Error processing attachment {f}: {e}")
